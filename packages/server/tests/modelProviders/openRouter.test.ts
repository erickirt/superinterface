...
describe('OpenRouter', () => {
  describe('clientAdapter', () => {
    it('returns valid adapter for OPEN_ROUTER model provider', async () => {
  ...
  describe('model provider config', () => {
    it('OpenRouter config supports SUPERINTERFACE_CLOUD storage', async () => {
      ...
      assert.ok(
        openRouterConfig.storageProviderTypes.includes(
          StorageProviderType.SUPERINTERFACE_CLOUD,
        ),
        'OpenRouter should support SUPERINTERFACE_CLOUD storage',
      )
    })
    ...
    it('OpenRouter config has function calling available', async () => {
      ...
      assert.ok(openRouterConfig, 'OpenRouter config should exist')
      assert.strictEqual(
        openRouterConfig.isFunctionCallingAvailable,
        true,
        'OpenRouter should have function calling available',
      )
    })
  })

  describe('integration', () => {
    it('should send message and receive response via OpenRouter', async () => {
      if (!openrouterApiKey) {
